...
*Polynomial.java
*
*The Polynomial class creates and stores a linked list of Terms for a given Polynomial object. Methods
*allow the user to add or remove terms from the linked list and clear the polynomial. Terms are ordered 
*from highest exponent to lowest, and exponents are compared so that terms can be combined or properly 
*added to the list.
*
...
public class Polynomial extends Term {
	...
	*/
    ...
    /**
    *Splits entered polynomial into individual Term objects and stores in an array. From these Terms, 
    *respective coefficients and exponents can be obtained.
    *
    *@param polynomial  the String polynomial that will be split into terms
    */
    public void splitPolynomial(String polynomial) {
        String[] termsInPoly = polynomial.replace(" ", "").split("(?=\\+|\\-)");

        for (int i = 0; i < termsInPoly.length; i++) {
        	Term term = new Term(termsInPoly[i]);
            terms.add(term);
        }
    }

    /**
    *Takes new entered term and compares to all terms in entered polynomial. Remove term from polynomial 
    *if it's a match.
    *
    ...
    */
    ...
    /**
    *Adds the entered term to a given polynomial. A new coefficient is calculated if the exponent matches 
    ...
    */
    public void addTerm(Term newTerm) {
        ...
        //If there is more than one term in the polynomial
          ...
          //Go through and compare to all terms except for the last term because there isn't a "nextTerm" for comparison for the final term
          for (int i = 0; i < this.getNumTerms() - 1; i++) {
            term = this.getTerm(i);
            nextTerm = this.getTerm(i+1);

            //If newTerm exponent is greater than the current term, add new Term at that index; return once term is added
            ...
            else if ((newTerm.compareTo(term) == -1) && (newTerm.compareTo(nextTerm) == 1)) {
    ...
    /**
    *Locates the selected term at the entered index and removes it from the polynomial.
    *
    ...
    */
    ...
    public String toString() {
        ...
        if(this.terms == null || this.terms.size() == 0) {
          ...
          if (finalPolynomial.charAt(0) == '+') {
            finalPolynomial = finalPolynomial.substring(1);
          }
        }
